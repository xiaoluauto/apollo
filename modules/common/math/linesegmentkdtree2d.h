...
/**
 * @file
 * @brief Defines the templated LineSegmentKDTree2dNode class.
 */
...
/**
 * @class LineSegmentKDTreeParams
 * @brief Contains parameters of axis-aligned bounding box.
 */
struct LineSegmentKDTreeParams {
...
class LineSegmentKDTree2dNode {
 public:
   ...
   */
  LineSegmentKDTree2dNode(const std::vector<ObjectPtr> &objects,
                          const LineSegmentKDTreeParams &params, int depth)
      : depth_(depth) {
    ...
    if (SplitToSubNodes(objects, params)) {
      ...
      if (!left_subnode_objects.empty()) {
        left_subnode_.reset(new LineSegmentKDTree2dNode<ObjectType>(
            left_subnode_objects, params, depth + 1));
      }
      if (!right_subnode_objects.empty()) {
        right_subnode_.reset(new LineSegmentKDTree2dNode<ObjectType>(
 ...
 private:
  void InitObjects(const std::vector<ObjectPtr> &objects) {
    ...
    std::sort(objects_sorted_by_min_.begin(), objects_sorted_by_min_.end(),
              [&](ObjectPtr obj1, ObjectPtr obj2) {
                return partition_ == PARTITION_X
                           ? fmin(obj1->start().x(), obj1->end().x()) <
                             fmin(obj2->start().x(), obj2->end().x())
                           : fmin(obj1->start().y(), obj1->end().y()) <
                             fmin(obj2->start().y(), obj2->end().y());
              });
    std::sort(objects_sorted_by_max_.begin(), objects_sorted_by_max_.end(),
              [&](ObjectPtr obj1, ObjectPtr obj2) {
                return partition_ == PARTITION_X
                           ? fmax(obj1->start().x(), obj1->end().x()) >
                             fmax(obj2->start().x(), obj2->end().x())
                           : fmax(obj1->start().y(), obj1->end().y()) >
                             fmax(obj2->start().y(), obj2->end().y());
              });
    ...
    for (ObjectPtr object : objects_sorted_by_min_) {
      ...
      partition_ == PARTITION_X ? fmin(object->start().x(), object->end().x())
                              : fmin(object->start().y(), object->end().y()));
    }
    ...
    for (ObjectPtr object : objects_sorted_by_max_) {
      ...
      partition_ == PARTITION_X ? fmax(object->start().x(), object->end().x())
                              : fmax(object->start().y(), object->end().y()));
    }
  }

  bool SplitToSubNodes(const std::vector<ObjectPtr> &objects,
                       const LineSegmentKDTreeParams &params) {
  ...
  void ComputeBoundary(const std::vector<ObjectPtr> &objects) {
    ...
    for (ObjectPtr object : objects) {
      min_x_ = std::fmin(min_x_, fmin(object->start().x(), object->end().x()));
      max_x_ = std::fmax(max_x_, fmax(object->start().x(), object->end().x()));
      min_y_ = std::fmin(min_y_, fmin(object->start().y(), object->end().y()));
      max_y_ = std::fmax(max_y_, fmax(object->start().y(), object->end().y()));
    }
  ...
  void PartitionObjects(const std::vector<ObjectPtr> &objects,
    ...
    if (partition_ == PARTITION_X) {
      for (ObjectPtr object : objects) {
        if (fmax(object->start().x(), object->end().x()) <=
            partition_position_) {
          left_subnode_objects->push_back(object);
        } else if (fmin(object->start().x(), object->end().x()) >=
                   partition_position_) {
      ...
      for (ObjectPtr object : objects) {
        if (fmax(object->start().y(), object->end().y()) <=
            partition_position_) {
          left_subnode_objects->push_back(object);
        } else if (fmin(object->start().y(), object->end().y()) >=
            partition_position_) {
          right_subnode_objects->push_back(object);
        } else {
          other_objects.push_back(object);
 ...
 private:
  ...
  std::unique_ptr<LineSegmentKDTree2dNode<ObjectType>> left_subnode_ = nullptr;
  std::unique_ptr<LineSegmentKDTree2dNode<ObjectType>> right_subnode_ = nullptr;
};

/**
 * @class LineSegmentKDTree2d
...
class LineSegmentKDTree2d {
 public:
   ...
   * @param params Parameters to build the KD-tree.
   */
  LineSegmentKDTree2d(const std::vector<ObjectType> &objects,
                const LineSegmentKDTreeParams &params) {
    if (!objects.empty()) {
      ...
      root_.reset(new LineSegmentKDTree2dNode<ObjectType>
                 (object_ptrs, params, 0));
    }
  }
 ...
 private:
  std::unique_ptr<LineSegmentKDTree2dNode<ObjectType>> root_ = nullptr;
};
